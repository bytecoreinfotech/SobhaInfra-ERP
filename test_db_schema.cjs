...
async function check() {
  ...
  console.log('Invoices query result:', JSON.stringify(invRes, null, 2));
  ...
  console.log('Insert test with invoice_number:', JSON.stringify(testIns, null, 2));
  ...
  console.log('Insert test with tally_voucher_number:', JSON.stringify(testIns2, null, 2));

  // Delete test inserts if any succeeded
  await supabase.from('invoices').delete().or('invoice_number.eq.TEST-001,tally_voucher_number.eq.TEST-002');
}
