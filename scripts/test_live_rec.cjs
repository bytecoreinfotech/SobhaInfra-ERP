...
// We can import or re-use reconcileVendorInvoices logic
async function run() {
  let allRows = [];
  let start = 0;
  const step = 1000;
  ...
  // Let's import the reconciliation code from src/lib/reconciliation.js
  const recCode = fs.readFileSync('src/lib/reconciliation.js', 'utf-8');
  // We can evaluate it by replacing exports
  const evalRec = new Function('exports', recCode.replace(/export\s+/g, ''));
  const rec = {};
  evalRec(rec);

  const reconciled = rec.reconcileVendorInvoices(allRows);
  ...
  const purchaseBills = actualSRP.filter(rec.isPurchaseVoucher);
